...
unsigned hash(char *s) {   // form hash value for string s 
    unsigned hashval;
...
struct nlist *install(char *name, char *defn) {
    ...
    if ((np = lookup(name)) == NULL) { // not found 
        np = (struct nlist *) malloc(sizeof(*np));   //allocate space 
...
int main() 
{
    struct nlist *table[4] = {
            (install("key", "apple")), (install("key1", "banana")),
            (install("key2", "mango")), (install("key3", "milk"))};
